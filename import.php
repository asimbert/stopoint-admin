...
//require_once(dirname(__FILE__) . '/classes/excel_reader2.php');
...
<div id="main-content"> <!-- Main Content Section with everything -->
			...
			<a href=""><h2>Import Products</h2></a>
...
{	
...

	{
		...
		$html.="<tr>";
			...
			$ProductCode = $eid = mysql_real_escape_string($data->sheets[$i][cells][$j][1]);
			...
			$Category = mysql_real_escape_string($data->sheets[$i][cells][$j][2]);
			...
			$vas = "SELECT * FROM `productcategory` WHERE Name='".$Category."'";
...
if(mysql_num_rows($re) == 1){
	...
	mysql_query("INSERT INTO productcategory (`Name`) VALUES('$Category')") or die(mysql_error());
	...
	$Category = mysql_insert_id();
			...
			$Brand = mysql_real_escape_string($data->sheets[$i][cells][$j][3]);

			$brand_name = $Brand;
			
			if($Brand != ""){

			$vasbrand = "SELECT * FROM `productbrand` WHERE Name='".$Brand."'";

$rebrand=mysql_query($vasbrand);

$webrand=mysql_fetch_assoc($rebrand);

if(mysql_num_rows($rebrand) == 1){
	...
	mysql_query("INSERT INTO productbrand (`Name`) VALUES('$Brand')") or die(mysql_error());

	$Brand = mysql_insert_id();
			...
			$Carrier = mysql_real_escape_string($data->sheets[$i][cells][$j][4]);
...
if($Carrier != ""){
			...
			$vas = "SELECT * FROM `carriers` WHERE Name='".$Carrier."'";

$recarrier=mysql_query($vas);

$wecarrier=mysql_fetch_assoc($recarrier);

if(mysql_num_rows($recarrier) == 1){

	$Carrier = $wecarrier['Id'];
	...
	mysql_query("INSERT INTO carriers (`Name`) VALUES('$Carrier')") or die(mysql_error());
	...
	$Carrier = mysql_insert_id();
			...
			$Family = mysql_real_escape_string($data->sheets[$i][cells][$j][5]);

			$family_name = $Family;

			if($Family != ""){

			$vasfamily = "SELECT * FROM `productfamily` WHERE Name='".$Family."' AND BrandId=".$Brand." AND CategoryId=".$Category;



$refamily=mysql_query($vasfamily);

$wefamily=mysql_fetch_assoc($refamily);

if(mysql_num_rows($refamily) == 1){
	...
	mysql_query("INSERT INTO productfamily(`Name`,`CategoryId`,`BrandId`) VALUES('$Family',$Category,$Brand)") or die(mysql_error());
	...
	$Family = mysql_insert_id();
			...
			$OrderNumber = mysql_real_escape_string($data->sheets[$i][cells][$j][6]);

			$ProductModel = mysql_real_escape_string($data->sheets[$i][cells][$j][7]);

			$Brokenyes = mysql_real_escape_string($data->sheets[$i][cells][$j][8]);

			$Brokenno = mysql_real_escape_string($data->sheets[$i][cells][$j][9]);

			$AdjustedPrice = mysql_real_escape_string($data->sheets[$i][cells][$j][10]);

			$GoodPrice = mysql_real_escape_string($data->sheets[$i][cells][$j][11]);

			

			$FlawlessPrice = mysql_real_escape_string($data->sheets[$i][cells][$j][12]);

			

			

			$Generation = mysql_real_escape_string($data->sheets[$i][cells][$j][13]);

			$StorageCapacity = mysql_real_escape_string($data->sheets[$i][cells][$j][14]);

			$CPU = mysql_real_escape_string($data->sheets[$i][cells][$j][15]);

			$ScreenSize = mysql_real_escape_string($data->sheets[$i][cells][$j][16]);

			$RAM = mysql_real_escape_string($data->sheets[$i][cells][$j][17]);

			$Band = mysql_real_escape_string($data->sheets[$i][cells][$j][18]);

			

			$Description = mysql_real_escape_string($data->sheets[$i][cells][$j][19]);

			$SubFamily = mysql_real_escape_string($data->sheets[$i][cells][$j][20]);

			$Image = mysql_real_escape_string($data->sheets[$i][cells][$j][21]);
			...
			$vascode = "SELECT * FROM `product` WHERE ProductCode='".$ProductCode."'";

$recode=mysql_query($vascode);

$wecode=mysql_fetch_assoc($recode);

if(mysql_num_rows($recode) == 1){

	$ProductCode = $wecode['id'];

	$sql = "UPDATE product SET CategoryId='".$Category."',BrandId='".$Brand."',CarrierId='".$Carrier."',FamilyId='".$Family."',OrderNumber='".$OrderNumber."',ProductModel='".$ProductModel."',brokenyes='".$Brokenyes."',brokenno='".$Brokenno."',GoodPrice='".$GoodPrice."',FlawessPrice='".$FlawlessPrice."',AcceptablePrice='".$AdjustedPrice."',Generation='".$Generation."',StorageCapacity='".$StorageCapacity."',CPU='".$CPU."',ScreenSize='".$ScreenSize."',RAM='".$RAM."',Band='".$Band."',Description='".$Description."',image_url='".$Image."' WHERE id=".$ProductCode;

	

	mysql_query($sql);
	
	$sqlfamilyimage = "UPDATE productfamily SET image_url='".$Image."',CategoryId='".$Category."',BrandId='".$Brand."' WHERE Id=".$Family;

	

	mysql_query($sqlfamilyimage);
	  ...
	  $query = 'insert into product(ProductCode,CategoryId,BrandId,CarrierId,FamilyId,OrderNumber,ProductModel,brokenyes,brokenno,GoodPrice,FlawessPrice,AcceptablePrice,Generation,StorageCapacity,CPU,ScreenSize,RAM,Band,Description,image_url) values("'.$ProductCode.'","'.$Category.'","'.$Brand.'","'.$Carrier.'","'.$Family.'","'.$OrderNumber.'","'.$ProductModel.'","'.$Brokenyes.'","'.$Brokenno.'","'.$GoodPrice.'","'.$FlawlessPrice.'","'.$AdjustedPrice.'","'.$Generation.'","'.$StorageCapacity.'","'.$CPU.'","'.$ScreenSize.'","'.$RAM.'","'.$Band.'","'.$Description.'","'.$Image.'")';

	  mysql_query($query);
	  
	//Check if this product is under category/brand which has some coupon code entry in the database.		
...
echo "<br />Products Inserted in database";
