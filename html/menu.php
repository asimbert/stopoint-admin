...
?>                        users_edit.php                        <?php
            }else if ($wesubroles['RoleName'] == 'View Users') {
?>                        customers.php                        <?php
            } else {
?>                        users.php                        <?php
            }
...
?>                    order.php?key=returncompleted&sort=DatePaid&desc                    <?php
            }else if ($wesubroles['RoleName'] == 'Release-Bulk Check Orders') {
?>                    paid-order.php                    <?php
...
?>                <li>                    <a href="#" class="nav-top-item">                        Import Database                    </a>                    <ul>                        <li><a href="import.php">Import Database</a></li>                    </ul>                </li>                                                                              
...
<?php
    $vassubroles = "SELECT * FROM `roles` INNER JOIN userroles on userroles.RoleId = roles.id INNER JOIN user on userroles.UserId = user.id WHERE ( userroles.UserId = " . $_SESSION['user_info']['id'] . " AND roles.ParentId = 51 )";
    $resubroles  = mysql_query($vassubroles);
?>                <?php
    if (mysql_num_rows($resubroles) == 0) {
    } else {
?>                                                                                 
		<li>                    
			<a href="#" class="nav-top-item">Front End Page</a>                    
			<ul>                        				
				 <?php
				 	while ($wesubroles = mysql_fetch_array($resubroles))
					{
						 if ($wesubroles['RoleName'] == 'Press Release') { ?>
							<li><a href="pressrelease.php"><?= $wesubroles['RoleName'] ?></a></li>
						<?php }
						if ($wesubroles['RoleName'] == 'Coupon Code') { ?>
							<li><a href="coupon_code.php"><?= $wesubroles['RoleName'] ?></a></li>
						<?php }
						?>
				<?php } ?>
			</ul>                
...
<?php
    $vassubroles = "SELECT * FROM `roles` LEFT JOIN userroles on userroles.RoleId = roles.id 
	WHERE ( userroles.UserId = ".$_SESSION['user_info']['id']." AND roles.ParentId = 58 )";
    $resubroles  = mysql_query($vassubroles);
	if (mysql_num_rows($resubroles) == 0) {
	} else { ?>
		 <li>
		 	<a href="#" class="nav-top-item">Printing</a> 
			<ul>
				 <?php
				 	while ($wesubroles = mysql_fetch_array($resubroles))
					{
						 if ($wesubroles['RoleName'] == 'Address Laberll Printing') { ?>
							<li><a href="address_lbl_printing.php"><?= $wesubroles['RoleName'] ?></a></li>
						<?php }
